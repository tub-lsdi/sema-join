...
def calculate_stats_cs(con: duckdb.DuckDBPyConnection) -> None:
    ...
    table_ids = (
        con.execute("SELECT DISTINCT table_id FROM cells").pl()["table_id"].to_list()
    )
    ...
    for i, current_table_id in enumerate(table_ids):
        ...
        if (i + 1) % 1000 == 0:
            logger.info(
                f"Processing table {i+1}/{len(table_ids)} (ID: {current_table_id})..."
            )
    ...
    logger.info(
        "Loop complete. Starting final aggregation. This will take some memory..."
    )
