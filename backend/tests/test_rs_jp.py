...
class TestRSJPAlgorithm(unittest.TestCase):
    ...
    def test_simple_maximization(self):
        ...
        scores = {
            ("r1", "s1"): 0.2, ("r1", "s2"): 0.8, ("r1", "s3"): 0.1,
            ("r2", "s1"): 0.9, ("r2", "s2"): 0.4, ("r2", "s3"): 0.3,
        }
        ...
        self.assertEqual(result_mapping["r1"], "s2",
                        "r1 should map to s2 (highest score 0.8)")
        self.assertEqual(result_mapping["r2"], "s1",
                        "r2 should map to s1 (highest score 0.9)")
    ...
    def test_many_to_one_behavior(self):
        ...
        # Assert
        result_mapping = {r["r_val"]: r["s_val"] for r in result}
        self.assertEqual(result_mapping["r1"], "s1",
                        "r1 should map to s1")
        self.assertEqual(result_mapping["r2"], "s1",
                        "r2 should also map to s1 (many-to-one)")
        
        # Verify that multiple R values can indeed map to the same S value
        s_values = [r["s_val"] for r in result]
        self.assertEqual(s_values.count("s1"), 2,
                        "s1 should appear twice (mapped by both r1 and r2)")
    ...
    def test_empty_r_input(self):
        ...
        # Assert
        self.assertEqual(len(result), 0,
                        "Empty R should produce empty result")
    ...
    def test_empty_s_input(self):
        ...
        # Assert
        self.assertEqual(len(result), 0,
                        "Empty S should produce no mappings (all ⊥)")
    ...
    def test_at_most_one_constraint(self):
        ...
        self.assertEqual(len(result_mapping), 3,
                        "Should have mappings for all 3 r values")
        
        # No r should map to multiple s values
        r_values = [r["r_val"] for r in result]
        self.assertEqual(len(r_values), len(set(r_values)),
    ...
    def test_independent_optimization(self):
        ...
        self.assertEqual(mapping_combined, expected_combined,
